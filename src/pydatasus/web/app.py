...
def page_status():
    ...
    try:
        ...
        try:
            ...
            st.info("Instale plotly para visualizar graficos: pip install plotly")
...
def page_query():
    ...
    try:
        ...
        # Two columns: templates and dictionary
        ...
        with col1:
            ...
            selected_template = st.selectbox(
                ...
                help="Selecione um template ou escreva sua propria consulta"
            )

            # Initialize query in session state if not exists
            if "current_query" not in st.session_state:
                st.session_state.current_query = f"""SELECT uf, COUNT(*) as total
FROM {subsystem}
GROUP BY uf
ORDER BY total DESC
LIMIT 10"""

            # Update query when template selected
        ...
        with col2:
            ...
            with st.expander("Ver descricao das colunas"):
        ...
        # Execute button
        if st.button("▶️ Executar", type="primary", disabled=not is_valid):
